...
/* const deviceEUI = data.devEui;
 ...
 const modelNumber = device.hardware.decoderName;

 // return console.log("check device details here ", modelNumber)
  
 console.log("decoderName", modelNumber);
 console.log("device found ", device)
 // return 
 if (!device) {
   throw new Error('Device not found');
...
//}
console.log('pdu  and modelNumber------------>',pdu,modelNumber);
switch (modelNumber) {
 case "LHT65Decoder":
   return decodePDUForTemperature(pdu); // Call specific decoding function for Model X
 case "LT2222Decoder":
...
const { device } = require('../config/database');
const { calculatePagination, filterTransactionsByDate } = require('../helper/commonHelper');
const { decodePDUByModel } = require('../helper/decodePDUByModel');
const { sendDownlinkTospecificServer } = require('../helper/sendDownlinkTospecificServer');
const {emitToMeteringIfApplicable} = require('../helper/webhookMetering');
...
const createDevice = async (data) => {
  ...
  if (data.serverName === 'Loriot') {
    try {
      ...
      const decoderName = device.hardware.decoderName;
      console.log("decoderName", decoderName);
      // return console.log("check device details here ", modelNumber)
      ...
      console.log("data.pdu", data.pdu, decoderName);

      const decodedData = decodePDUByModel(data.pdu, decoderName);
      console.log("decodedData", decodedData);
      console.log("decodedData", decodedData === false ? "enegery temp pdu" : "else decoder");
      ...
      // if (device.hardware.type === 'meter') {
      //   //check for the token is present or not of metering solution.
      //   //first make the array of single decoded data object
      //   const arrayOfDecodedData = [decodedData];
      //   for (const decodedData of arrayOfDecodedData) {
      //     //chcek token is present or not
      //   const token = await prisma.meteringtoken.findFirst({
      //     where: {
      //       deviceId: device.id
      //     }
      //   });
      //   if (!token || token === null) {
      //     logger.error('Metering token not found for device:', device.id);

      //   }

      //   //if the token is presnt then use Webhook to send the data to the metering solution
      //   //function of webhook is in helper folder
      //   const webhookResponse = await emitToMeteringIfApplicable(decodedData, token,);
      ...
      //   }
      //   else {
      //     logger.error('Webhook response failed:', webhookResponse);
      //     continue; // Skip to the next iteration if webhook fails
      //   }
      //   }
      //   arrayOfDecodedData = [];
      ...
      // }
      if (device.hardware.type === 'meter') {
        // Wrap single decodedData in array (for consistency/future-proofing)
        const arrayOfDecodedData = [decodedData];

        for (const data of arrayOfDecodedData) {
          ...
          const webhookResponse = await emitToMeteringIfApplicable(data);

          if (webhookResponse?.status === 'success') {
            logger.info('Webhook success:', webhookResponse);
          } else {
            logger.error('Webhook failed:', webhookResponse);
          }
        }
      }
      ...
      // Run the threshold check service asynchronously, now passing deviceId, timestamp, and decodedData
      checkThresholdService(device.id, timestamp, decodedData, customerDetails, device.name, device.deviceId, data.serverName)
        .then(() => logger.info(`Threshold check for device ${device.id} completed.`))
        .catch((error) => logger.error('Error running threshold check:', error));
      // Return the response before the threshold check finishes
      return { device: newDevice, decodedData: newDecodeEntries };
    ...
    console.log("this is not resgisterd");
  }

};

async function checkThresholdService(deviceId, timestamp, decodedData, customerDetails, deviceName, deviceEui, serverName) {
  try {
    console.log("helo 1 ", customerDetails, deviceName, deviceEui)
    logger.info(`Checking thresholds for device: ${deviceId} at timestamp: ${timestamp}`);
    ...
    console.log("threshold agya hai", thresholds);
    // Check if any decoded attributes exceed the threshold limits
    const exceedingAttributes = await checkThreshold(decodedData, thresholds, serverName, deviceEui);
    console.log("exceedingAttributes", exceedingAttributes)
...
async function checkThreshold(decodedData, alerts, serverName, deviceEui) {
  console.log("Decoded Data:", decodedData);
  console.log("Alerts:", alerts);
  ...
  for (const alert of alerts) {
    ...
    if (!isNaN(attributeValue)) {
      ...
      if (conditionMet) {
        ...
        // if condito is met then send the downlink
        const response = sendDownlinkTospecificServer(serverName, alert, deviceEui);
        console.log("Sending downlink through the function", response);
